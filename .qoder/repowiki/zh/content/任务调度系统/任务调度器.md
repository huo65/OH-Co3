# 任务调度器

<cite>
**本文引用的文件**
- [Scheduler.ets](file://entry/src/main/ets/manager/scheduler/Scheduler.ets)
- [TaskDispatch.ets](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets)
- [SystemProfiler.ets](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets)
- [ParamSyn.ets](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets)
- [NodeInfoSyn.ets](file://entry/src/main/ets/manager/broker/monitor/NodeInfoSyn.ets)
- [InferenceWorker.ets](file://entry/src/main/ets/manager/worker/InferenceWorker.ets)
- [GlobalTaskManager.ets](file://entry/src/main/ets/manager/worker/GlobalTaskManager.ets)
- [SerialTaskQueue.ets](file://entry/src/main/ets/manager/worker/SerialTaskQueue.ets)
- [MQTTClient.ets](file://entry/src/main/ets/manager/broker/MQTTClient.ets)
- [HarmonyInferenceClient.ets](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets)
- [TransferDataModels.ets](file://entry/src/main/ets/manager/transfer/model/TransferDataModels.ets)
</cite>

## 更新摘要
**所做更改**
- 新增重试配置管理章节，详细介绍 SchedulerRetryConfig 接口和配置选项
- 新增重试队列项处理章节，说明 SchedulerRetryQueueItem 的结构和处理流程
- 新增失败类型分类章节，涵盖 SchedulerFailureType 枚举的六种失败场景
- 新增指数退避算法支持章节，详细说明 calculateRetryDelay 方法的实现
- 更新任务状态管理系统，增加 RETRYING 状态和重试相关字段
- 完善重试机制的完整实现，包括调度重试、处理重试、手动重试等功能
- **更新**：移除重复的状态转换记录代码，优化任务状态管理的简洁性和可靠性

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [任务状态管理系统](#任务状态管理系统)
7. [重试机制](#重试机制)
8. [依赖关系分析](#依赖关系分析)
9. [性能考量](#性能考量)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
12. [附录](#附录)

## 简介
本文件面向开发者，系统化阐述任务调度器的实现与使用，重点覆盖以下方面：
- Scheduler 类的核心职责：设备信息收集、节点评分计算、任务决策与分发
- calculate_score 评分算法的原理与权重来源
- 单例模式与 getInstance 的作用
- dispatchTask 的任务分发机制与 TaskTransmitData 的封装流程
- **新增**：完整的任务状态管理系统，包括8种任务状态、状态跟踪、历史记录和实时通知
- **新增**：智能重试机制，支持指数退避和多种失败类型的处理
- **新增**：重试配置管理、重试队列项处理、失败类型分类的完整实现
- **更新**：移除重复的状态转换记录代码，优化状态管理的简洁性和可靠性
- 使用示例与最佳实践，帮助正确地进行任务分配与结果处理

## 项目结构
任务调度器位于 entry 模块的 manager/scheduler 目录下，围绕设备信息采集、参数同步、任务分发与执行队列形成闭环。MQTT 作为消息通道，负责节点状态同步、参数下发与任务/结果消息的传输。**新增**的任务状态管理系统和**新增**的智能重试机制为整个调度流程提供了完整的可观测性和可靠性保障。

```mermaid
graph TB
subgraph "调度层"
S["Scheduler<br/>任务调度器"]
TS["任务状态管理<br/>8种状态跟踪"]
RM["重试机制<br/>指数退避"]
RC["重试配置<br/>SchedulerRetryConfig"]
RQ["重试队列<br/>SchedulerRetryQueueItem"]
FT["失败类型<br/>SchedulerFailureType"]
end
subgraph "监控与参数"
SP["SystemProfiler<br/>系统信息采集"]
NI["NodeInfoSyn<br/>节点信息同步"]
PS["ParamSyn<br/>参数同步"]
end
subgraph "传输层"
TD["TaskTransmitData<br/>任务传输数据"]
TD2["TaskResTransmitData<br/>结果传输数据"]
TD3["TaskDispatch<br/>任务分发"]
MQTT["MQTTClient<br/>消息客户端"]
end
subgraph "执行层"
GTM["GlobalTaskManager<br/>全局任务队列"]
STQ["SerialTaskQueue<br/>串行任务队列"]
IW["InferenceWorker<br/>推理Worker"]
end
S --> SP
S --> NI
S --> PS
S --> TS
S --> RM
S --> RC
S --> RQ
S --> FT
S --> TD
S --> TD3
TD3 --> MQTT
TD3 --> TD2
TD3 --> GTM
GTM --> STQ
STQ --> IW
```

**图表来源**
- [Scheduler.ets:141-187](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L141-L187)
- [SystemProfiler.ets:43-120](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets#L43-L120)
- [NodeInfoSyn.ets:40-173](file://entry/src/main/ets/manager/broker/monitor/NodeInfoSyn.ets#L40-L173)
- [ParamSyn.ets:13-40](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L13-L40)
- [TaskDispatch.ets:98-302](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L98-L302)
- [GlobalTaskManager.ets:4-27](file://entry/src/main/ets/manager/worker/GlobalTaskManager.ets#L4-L27)
- [SerialTaskQueue.ets:13-104](file://entry/src/main/ets/manager/worker/SerialTaskQueue.ets#L13-L104)
- [InferenceWorker.ets:1-90](file://entry/src/main/ets/manager/worker/InferenceWorker.ets#L1-L90)
- [MQTTClient.ets:24-223](file://entry/src/main/ets/manager/broker/MQTTClient.ets#L24-L223)

**章节来源**
- [Scheduler.ets:1-105](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1-L105)
- [SystemProfiler.ets:1-120](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets#L1-L120)
- [TaskDispatch.ets:1-302](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L1-L302)

## 核心组件
- 调度器 Scheduler：负责收集设备信息、计算节点评分、做出本地/远程执行决策，并在需要时分发任务
- **新增**：任务状态管理系统：跟踪每个任务的完整生命周期状态，提供实时状态查询和历史记录
- **新增**：智能重试机制：支持指数退避策略，自动处理各种失败场景，包括调度失败、本地执行失败、远程执行失败、传输失败、结果接收超时等
- **新增**：重试配置管理：提供 SchedulerRetryConfig 接口，支持最大重试次数、基础延迟时间、最大延迟时间和指数退避开关的配置
- **新增**：重试队列项处理：使用 SchedulerRetryQueueItem 结构管理待重试的任务，包括任务数据、重试次数、计划执行时间和失败原因
- **新增**：失败类型分类：通过 SchedulerFailureType 枚举区分不同类型的失败场景，便于针对性处理
- 传输模块 TaskDispatch：封装任务数据、选择传输协议（MQTT 或 TCP）、发送任务与结果
- 系统信息 SystemProfiler：聚合本机与其它节点的设备信息
- 参数同步 ParamSyn：维护评分权重参数，支持运行时更新
- 执行队列 GlobalTaskManager/SerialTaskQueue：串行执行本地推理任务
- MQTTClient：统一的 MQTT 客户端，负责订阅/发布与事件派发
- HarmonyInferenceClient：对外暴露的客户端入口，协调初始化、事件监听与任务提交

**章节来源**
- [Scheduler.ets:14-105](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L14-L105)
- [TaskDispatch.ets:98-302](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L98-L302)
- [SystemProfiler.ets:43-120](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets#L43-L120)
- [ParamSyn.ets:13-40](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L13-L40)
- [GlobalTaskManager.ets:4-27](file://entry/src/main/ets/manager/worker/GlobalTaskManager.ets#L4-L27)
- [SerialTaskQueue.ets:13-104](file://entry/src/main/ets/manager/worker/SerialTaskQueue.ets#L13-L104)
- [MQTTClient.ets:24-223](file://entry/src/main/ets/manager/broker/MQTTClient.ets#L24-L223)
- [HarmonyInferenceClient.ets:52-357](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L52-L357)

## 架构总览
调度器工作流概览如下：

```mermaid
sequenceDiagram
participant Client as "调用方"
participant HIC as "HarmonyInferenceClient"
participant SCH as "Scheduler"
participant TS as "任务状态管理"
participant RM as "重试机制"
participant SP as "SystemProfiler"
participant NI as "NodeInfoSyn"
participant PS as "ParamSyn"
participant TD as "TaskDispatch"
participant MQTT as "MQTTClient"
participant GTM as "GlobalTaskManager"
participant STQ as "SerialTaskQueue"
participant IW as "InferenceWorker"
Client->>HIC : "submitTask(task)"
HIC->>SCH : "workScheduler(task, log)"
SCH->>TS : "initTaskStatus(taskId, task)"
TS->>TS : "记录PENDING状态"
SCH->>SP : "getDeviceInfos()"
SP->>NI : "getOtherNodeInfo()/sendOwnInfo()"
SCH->>PS : "getNewParam()"
SCH->>SCH : "calculate_score(node)"
alt "最优节点不是本机"
SCH->>TS : "updateTaskStatus(DISPATCHED)"
SCH->>TD : "dispatchTask(task, to)"
TD->>MQTT : "publish('/task/assign', TaskTransmitData)"
MQTT-->>HIC : "事件派发(/task/assign)"
HIC->>TD : "parseTaskMessage(message)"
TD->>GTM : "addTask(task)"
GTM->>STQ : "processTask()"
STQ->>IW : "infer(task)"
IW-->>STQ : "InferenceResult"
STQ-->>TD : "TaskResTransmitData"
TD->>MQTT : "publish('/task/result', TaskResTransmitData)"
MQTT-->>HIC : "事件派发(/task/result)"
HIC->>SCH : "updateRemoteTaskStatus"
SCH->>TS : "updateTaskStatus(COMPLETED/FAILED)"
alt "执行失败"
SCH->>RM : "scheduleRetry(taskId, failureType, reason)"
RM->>RM : "calculateRetryDelay(retryCount)"
RM->>RM : "加入重试队列"
RM->>RM : "启动重试处理器"
end
TS->>TS : "记录流转历史"
HIC->>HIC : "handleTaskResult -> 任务完成"
else "最优节点是本机"
SCH->>TS : "updateTaskStatus(RUNNING_LOCAL)"
SCH-->>HIC : "返回本地执行"
HIC->>GTM : "addTask(task)"
GTM->>STQ : "processTask()"
STQ->>IW : "infer(task)"
IW-->>STQ : "InferenceResult"
STQ-->>HIC : "返回结果"
HIC->>SCH : "updateLocalTaskStatus"
SCH->>TS : "updateTaskStatus(COMPLETED/FAILED)"
else "执行失败"
SCH->>RM : "scheduleRetry(taskId, failureType, reason)"
RM->>RM : "calculateRetryDelay(retryCount)"
RM->>RM : "加入重试队列"
RM->>RM : "启动重试处理器"
end
```

**图表来源**
- [HarmonyInferenceClient.ets:309-353](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L309-L353)
- [Scheduler.ets:30-57](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L30-L57)
- [SystemProfiler.ets:47-81](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets#L47-L81)
- [NodeInfoSyn.ets:40-80](file://entry/src/main/ets/manager/broker/monitor/NodeInfoSyn.ets#L40-L80)
- [ParamSyn.ets:35-37](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L35-L37)
- [TaskDispatch.ets:107-126](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L107-L126)
- [MQTTClient.ets:148-182](file://entry/src/main/ets/manager/broker/MQTTClient.ets#L148-L182)
- [GlobalTaskManager.ets:16-21](file://entry/src/main/ets/manager/worker/GlobalTaskManager.ets#L16-L21)
- [SerialTaskQueue.ets:51-83](file://entry/src/main/ets/manager/worker/SerialTaskQueue.ets#L51-L83)
- [InferenceWorker.ets:76-89](file://entry/src/main/ets/manager/worker/InferenceWorker.ets#L76-L89)

## 详细组件分析

### Scheduler 类：设备信息收集、评分与决策
- 设备信息收集
  - 通过 SystemProfiler.getDeviceInfos() 聚合本机与其它节点的 DeviceInfo
  - 本机信息由 SystemProfiler.getOwnInfo() 采集，包括 CPU、内存、存储、电池与网络延迟
  - 其它节点信息由 NodeInfoSyn 维护，周期性通过 MQTT 广播/订阅更新
- 节点评分计算
  - 调用 calculate_score(nodeInfo)，使用 ParamSyn.getNewParam() 获取权重参数
  - 评分公式综合考虑：CPU 使用率、内存使用率、电池电量、存储空间、网络延迟
  - 评分越高表示节点越适合执行任务
- 任务决策
  - 遍历所有节点，选择评分最高的节点
  - 若最高分节点不是本机，则调用 dispatchTask 分发任务；否则本地执行
- 单例模式
  - 提供 getInstance()，保证全局唯一调度实例，避免重复初始化与状态不一致

```mermaid
flowchart TD
Start(["进入 workScheduler"]) --> InitStatus["初始化任务状态<br/>initTaskStatus(taskId, task)"]
InitStatus --> UpdatePending["更新状态为PENDING<br/>记录初始历史"]
UpdatePending --> Collect["收集设备信息<br/>systemProfiler.getDeviceInfos()"]
Collect --> InitBest["初始化最高分节点"]
InitBest --> Loop{"遍历每个节点"}
Loop --> |计算评分| Score["calculate_score(node)"]
Score --> Compare{"是否高于当前最高分？"}
Compare --> |是| Update["更新最高分节点"]
Compare --> |否| Next["下一个节点"]
Update --> Next
Next --> |循环结束| Decide{"存在最高分节点？"}
Decide --> |否| Log["无节点可用，记录日志"] --> ReturnLocal["返回本地执行(true)"]
Decide --> |是| CheckOwn{"最高分节点是否为本机？"}
CheckOwn --> |是| UpdateLocal["更新状态为RUNNING_LOCAL<br/>记录历史"] --> ReturnLocal
CheckOwn --> |否| UpdateDispatched["更新状态为DISPATCHED<br/>记录历史"] --> Dispatch["dispatchTask(task, to)"] --> ReturnRemote["返回远端执行(false)"]
```

**图表来源**
- [Scheduler.ets:208-281](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L208-L281)
- [SystemProfiler.ets:47-81](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets#L47-L81)
- [ParamSyn.ets:35-37](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L35-L37)

**章节来源**
- [Scheduler.ets:14-105](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L14-L105)
- [SystemProfiler.ets:43-120](file://entry/src/main/ets/manager/monitor/SystemProfiler.ets#L43-L120)
- [NodeInfoSyn.ets:40-173](file://entry/src/main/ets/manager/broker/monitor/NodeInfoSyn.ets#L40-L173)
- [ParamSyn.ets:13-40](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L13-L40)

### calculate_score 评分算法原理
- 输入：节点 DeviceInfo，包含 cpuUsage、memoryUsage、batteryLevel、storageFree、latency
- 权重：ParamSyn.getNewParam() 返回 [w1, w2, w3, w4, w5]
- 计算：
  - score = w1*(1 - cpuUsage) + w2*(1 - memoryUsage) + w3*batteryLevel + w4*storageFree + w5*(1 - latency)
- 解释：
  - CPU/内存使用率取 1 - usage，越低评分越高
  - 电池电量与存储空间直接加分
  - 网络延迟取 1 - latency，越低越好
- 参数来源与更新
  - 默认权重 [1,1,1,1,1]
  - 通过 MQTT 主题 /optimization/param 接收参数更新，ParamSyn.parseParamMessage 解析并替换当前权重

```mermaid
flowchart TD
Enter(["进入 calculate_score"]) --> GetParam["获取权重参数<br/>paramSyn.getNewParam()"]
GetParam --> Compute["按公式加权求和"]
Compute --> Return["返回 score"]
```

**图表来源**
- [Scheduler.ets:66-70](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L66-L70)
- [ParamSyn.ets:25-37](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L25-L37)

**章节来源**
- [Scheduler.ets:66-70](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L66-L70)
- [ParamSyn.ets:13-40](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L13-L40)

### 单例模式与 getInstance
- Scheduler 采用静态字段保存唯一实例
- getInstance() 在首次调用时创建实例，后续直接返回
- 作用：确保调度策略与状态在应用生命周期内一致，避免重复初始化带来的资源浪费与竞态

**章节来源**
- [Scheduler.ets:76-81](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L76-L81)

### dispatchTask 任务分发机制与 TaskTransmitData 封装
- TaskTransmitData 字段
  - taskId、fromClient、toClient、timestamp、params（推理任务）、可选 sourceIp、transferType、fileHash
- 封装流程
  - 读取系统时间戳，填充 TaskTransmitData
  - 通过 TaskDispatch.sendTask(taskInfo) 发送
- 传输协议选择
  - 小文件：直接通过 MQTT 主题 /task/assign 发布
  - 大文件：TaskDispatch 内部切换 TCP 传输，设置 transferType='tcp' 并携带 sourceIp
- 接收与执行
  - 目标设备收到消息后，若为本地执行则加入 GlobalTaskManager 的 SerialTaskQueue，由 InferenceWorker 执行推理
  - 执行完成后通过 /task/result 回传结果

```mermaid
classDiagram
class TaskTransmitData {
+string taskId
+string fromClient
+string toClient
+number timestamp
+InferenceTask params
+string sourceDevice
+string transferType
+string fileHash
}
class TaskDispatch {
+sendTask(taskInfo) Promise~boolean~
+parseTaskMessage(message) Promise~void~
+sendTaskResult(result) void
+parseResultMessage(message) TaskResTransmitData | null
}
class MQTTClient {
+pushMessage(msg, topic) Promise~void~
+getInstance() MQTTClient
}
TaskDispatch --> TaskTransmitData : "封装/解析"
TaskDispatch --> MQTTClient : "发布/订阅"
```

**图表来源**
- [TaskDispatch.ets:16-92](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L16-L92)
- [TaskDispatch.ets:107-126](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L107-L126)
- [TaskDispatch.ets:187-212](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L187-L212)
- [TaskDispatch.ets:272-281](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L272-L281)
- [MQTTClient.ets:190-203](file://entry/src/main/ets/manager/broker/MQTTClient.ets#L190-L203)

**章节来源**
- [TaskDispatch.ets:16-92](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L16-L92)
- [TaskDispatch.ets:107-126](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L107-L126)
- [TaskDispatch.ets:187-212](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L187-L212)
- [TaskDispatch.ets:272-281](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L272-L281)

### 执行队列与本地推理
- GlobalTaskManager.init(worker) 初始化全局串行队列
- SerialTaskQueue.addTask(task) 非阻塞入队，首个任务触发 processTask
- processTask 串行执行，调用 InferenceWorker.infer(task) 获取结果
- 队列为空或异常时自动恢复，支持 shutdown 清空队列

**章节来源**
- [GlobalTaskManager.ets:4-27](file://entry/src/main/ets/manager/worker/GlobalTaskManager.ets#L4-L27)
- [SerialTaskQueue.ets:13-104](file://entry/src/main/ets/manager/worker/SerialTaskQueue.ets#L13-L104)
- [InferenceWorker.ets:76-89](file://entry/src/main/ets/manager/worker/InferenceWorker.ets#L76-L89)

## 任务状态管理系统

### 8种任务状态枚举
SchedulerTaskStatus 定义了任务在调度生命周期中的完整状态集合：

- **PENDING**：任务已创建，等待调度决策
- **SCHEDULING**：正在评估节点性能，进行调度决策  
- **DISPATCHED**：任务已分发到远程节点
- **RUNNING_LOCAL**：任务正在本地执行
- **RUNNING_REMOTE**：任务正在远程节点执行
- **COMPLETED**：任务执行完成
- **FAILED**：任务执行失败
- **CANCELLED**：任务已取消
- **RETRYING**：任务等待重试

```mermaid
stateDiagram-v2
[*] --> PENDING
PENDING --> SCHEDULING : 开始调度
SCHEDULING --> DISPATCHED : 选择远程节点
SCHEDULING --> RUNNING_LOCAL : 选择本地节点
DISPATCHED --> RUNNING_REMOTE : 远程执行开始
RUNNING_LOCAL --> COMPLETED : 本地执行成功
RUNNING_LOCAL --> FAILED : 本地执行失败
RUNNING_REMOTE --> COMPLETED : 远程执行成功
RUNNING_REMOTE --> FAILED : 远程执行失败
PENDING --> CANCELLED : 取消任务
FAILED --> RETRYING : 触发重试
RETRYING --> PENDING : 重试执行
COMPLETED --> [*]
FAILED --> [*]
CANCELLED --> [*]
```

**图表来源**
- [Scheduler.ets:13-32](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L13-L32)

### 任务状态跟踪映射
- **taskStatusMap**：存储所有被调度任务的状态信息，key为taskId
- **状态监听器**：支持注册多个监听器，实时通知外部状态变更
- **状态更新机制**：每次状态变更都会记录到 transitionHistory 中

### 任务流转历史记录
- **TaskTransitionRecord**：记录任务状态变更的历史轨迹
- **字段**：timestamp、status、node、message
- **用途**：提供完整的任务生命周期追踪，便于调试和分析

### 实时状态查询接口
- **getTaskStatus(taskId)**：获取指定任务的状态信息
- **getAllTaskStatus()**：获取所有任务的状态映射
- **getTasksByStatus(status)**：按状态筛选任务列表
- **getTaskTransitionHistory(taskId)**：获取任务的流转历史记录

**章节来源**
- [Scheduler.ets:102-129](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L102-L129)
- [Scheduler.ets:316-467](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L316-L467)
- [Scheduler.ets:432-467](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L432-L467)

### **更新**：移除重复的状态转换记录代码
在任务状态初始化过程中，系统已经通过构造函数中的初始历史记录确保了任务状态的正确初始化。之前存在的重复状态转换记录代码已被移除，避免了状态历史记录的冗余，提高了状态管理的简洁性和可靠性。

**更新**：优化后的状态初始化流程
- 通过 `initTaskStatus` 方法创建任务状态时，已经在构造函数中添加了初始的 PENDING 状态记录
- 移除了注释掉的重复状态转换记录代码，避免了不必要的历史记录条目
- 保持了状态历史记录的完整性和准确性，同时减少了冗余操作

**章节来源**
- [Scheduler.ets:316-467](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L316-L467)
- [Scheduler.ets:355-361](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L355-L361)

## 重试机制

### 重试配置管理
SchedulerRetryConfig 接口定义了重试机制的所有配置选项：

- **maxRetries**：最大重试次数，默认3次
- **baseDelayMs**：基础延迟时间，默认1000ms  
- **maxDelayMs**：最大延迟时间，默认10000ms
- **exponentialBackoff**：是否启用指数退避，默认true

提供以下公共方法：
- **setRetryConfig(config: Partial<SchedulerRetryConfig>)**：动态设置重试配置
- **getRetryConfig()**：获取当前重试配置

```mermaid
classDiagram
class SchedulerRetryConfig {
+number maxRetries
+number baseDelayMs
+number maxDelayMs
+boolean exponentialBackoff
}
class Scheduler {
+setRetryConfig(config) void
+getRetryConfig() SchedulerRetryConfig
}
Scheduler --> SchedulerRetryConfig : "配置管理"
```

**图表来源**
- [Scheduler.ets:47-56](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L47-L56)
- [Scheduler.ets:753-781](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L753-L781)

**章节来源**
- [Scheduler.ets:47-56](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L47-L56)
- [Scheduler.ets:753-781](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L753-L781)

### 重试队列项处理
SchedulerRetryQueueItem 结构用于管理待重试的任务：

- **task**：待重试的任务对象
- **retryCount**：当前重试次数
- **scheduledTime**：计划执行时间戳
- **failureReason**：失败原因描述

重试队列管理功能：
- **retryQueue**：存储待重试的任务数组
- **activeRetryCount**：当前活跃重试任务计数
- **startRetryProcessor()**：启动重试处理器，定时检查队列
- **clearRetryQueue()**：清理重试队列和处理器

```mermaid
flowchart TD
Queue["重试队列<br/>SchedulerRetryQueueItem[]"] --> Item["重试队列项<br/>task, retryCount, scheduledTime, failureReason"]
Item --> Processor["重试处理器<br/>定时器检查队列"]
Processor --> Check{"到期任务？"}
Check --> |是| Execute["执行重试"]
Check --> |否| Wait["等待下次检查"]
Execute --> Update["更新任务状态"]
Update --> Callback["调用重试回调"]
```

**图表来源**
- [Scheduler.ets:62-71](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L62-L71)
- [Scheduler.ets:186-191](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L186-L191)
- [Scheduler.ets:910-979](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L910-L979)

**章节来源**
- [Scheduler.ets:62-71](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L62-L71)
- [Scheduler.ets:186-191](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L186-L191)
- [Scheduler.ets:910-979](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L910-L979)

### 失败类型分类
SchedulerFailureType 枚举定义了六种不同的失败场景：

- **SCHEDULING_FAILED**：调度决策失败
- **LOCAL_EXECUTION_FAILED**：本地执行失败
- **REMOTE_EXECUTION_FAILED**：远程执行失败
- **TRANSMISSION_FAILED**：任务传输失败
- **RESULT_TIMEOUT**：结果接收超时
- **UNKNOWN**：未知错误

每种失败类型都有对应的处理方法：
- **handleSchedulingFailure(taskId, error)**：处理调度失败
- **handleLocalExecutionFailure(taskId, error)**：处理本地执行失败
- **handleRemoteExecutionFailure(taskId, error)**：处理远程执行失败
- **handleTransmissionFailure(taskId, error)**：处理传输失败
- **handleResultTimeout(taskId)**：处理结果接收超时

```mermaid
classDiagram
class SchedulerFailureType {
<<enumeration>>
+SCHEDULING_FAILED
+LOCAL_EXECUTION_FAILED
+REMOTE_EXECUTION_FAILED
+TRANSMISSION_FAILED
+RESULT_TIMEOUT
+UNKNOWN
}
class Scheduler {
+handleSchedulingFailure()
+handleLocalExecutionFailure()
+handleRemoteExecutionFailure()
+handleTransmissionFailure()
+handleResultTimeout()
}
Scheduler --> SchedulerFailureType : "失败类型处理"
```

**图表来源**
- [Scheduler.ets:77-90](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L77-L90)
- [Scheduler.ets:1082-1119](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1082-L1119)

**章节来源**
- [Scheduler.ets:77-90](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L77-L90)
- [Scheduler.ets:1082-1119](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1082-L1119)

### 指数退避算法支持
calculateRetryDelay 方法实现了智能的延迟计算算法：

**算法原理**：
- **指数退避**：delay = baseDelay × 1.5^retryCount
- **随机抖动**：0.8-1.2倍的随机延迟，避免雪崩效应
- **最大延迟限制**：防止重试间隔无限增长

**实现细节**：
- 支持可配置的基础延迟时间
- 支持最大延迟时间限制
- 支持禁用指数退避，使用固定延迟
- 自动添加随机抖动，提高系统稳定性

```mermaid
flowchart TD
Start(["计算重试延迟"]) --> CheckExp{"启用指数退避？"}
CheckExp --> |是| Exp["指数退避：delay = baseDelay * 1.5^retryCount"]
CheckExp --> |否| Fixed["固定延迟：使用基础延迟"]
Exp --> Jitter["添加随机抖动0.8-1.2倍"]
Fixed --> Jitter
Jitter --> Limit["应用最大延迟限制"]
Limit --> Return["返回最终延迟时间"]
```

**图表来源**
- [Scheduler.ets:821-837](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L821-L837)

**章节来源**
- [Scheduler.ets:821-837](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L821-L837)

### 重试调度与处理流程
完整的重试机制包括以下关键步骤：

1. **调度重试**：scheduleRetry 方法处理任务失败并安排重试
2. **计算延迟**：calculateRetryDelay 根据重试次数计算延迟时间
3. **更新状态**：将任务状态更新为 RETRYING，记录历史
4. **加入队列**：将任务加入重试队列，设置计划执行时间
5. **启动处理器**：启动重试处理器定时检查队列
6. **执行重试**：到期后通过回调重新提交任务
7. **完成通知**：重试完成后通知处理器减少活跃计数

```mermaid
sequenceDiagram
participant SCH as "Scheduler"
participant RM as "重试机制"
participant RP as "重试处理器"
participant CB as "重试回调"
SCH->>RM : "scheduleRetry(taskId, failureType, reason)"
RM->>RM : "calculateRetryDelay(retryCount)"
RM->>RM : "更新状态为RETRYING"
RM->>RM : "加入重试队列"
RM->>RP : "启动处理器"
RP->>RP : "定时检查队列"
RP->>CB : "调用重试回调"
CB->>SCH : "重新提交任务"
SCH->>RM : "notifyRetryComplete(taskId)"
RM->>RM : "减少活跃计数"
```

**图表来源**
- [Scheduler.ets:846-903](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L846-L903)
- [Scheduler.ets:910-979](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L910-L979)
- [Scheduler.ets:986-991](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L986-L991)

**章节来源**
- [Scheduler.ets:846-903](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L846-L903)
- [Scheduler.ets:910-979](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L910-L979)
- [Scheduler.ets:986-991](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L986-L991)

### 手动重试与状态管理
- **canRetry(taskId)**：判断任务是否可以重试，基于重试次数和最大重试限制
- **retryTask(taskId)**：手动触发任务重试，适用于 FAILED 状态的任务
- **getTaskRetryCount(taskId)**：获取任务当前重试次数
- **notifyRetryComplete(taskId)**：通知重试任务完成，用于资源清理

**章节来源**
- [Scheduler.ets:797-813](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L797-L813)
- [Scheduler.ets:1039-1056](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1039-L1056)
- [Scheduler.ets:986-991](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L986-L991)

## 依赖关系分析
- Scheduler 依赖 SystemProfiler、NodeInfoSyn、ParamSyn、TaskDispatch、InferenceTask、systemDateTime
- **新增**：任务状态管理依赖 systemDateTime 进行时间戳记录
- **新增**：重试机制依赖 systemDateTime 和定时器进行延迟控制
- **新增**：重试配置管理依赖 SchedulerRetryConfig 接口
- **新增**：重试队列项处理依赖 SchedulerRetryQueueItem 结构
- **新增**：失败类型分类依赖 SchedulerFailureType 枚举
- TaskDispatch 依赖 MQTTClient、GlobalTaskManager、SerialTaskQueue、InferenceWorker、NetworkUtils、FileUtils
- HarmonyInferenceClient 作为门面，串联 MQTT、调度器、执行队列与事件监听

```mermaid
graph LR
SCH["Scheduler"] --> SP["SystemProfiler"]
SCH --> NI["NodeInfoSyn"]
SCH --> PS["ParamSyn"]
SCH --> TD["TaskDispatch"]
SCH --> TS["任务状态管理"]
SCH --> RM["重试机制"]
SCH --> RC["重试配置"]
SCH --> RQ["重试队列"]
SCH --> FT["失败类型"]
TD --> MQTT["MQTTClient"]
TD --> GTM["GlobalTaskManager"]
GTM --> STQ["SerialTaskQueue"]
STQ --> IW["InferenceWorker"]
HIC["HarmonyInferenceClient"] --> SCH
HIC --> TD
HIC --> MQTT
```

**图表来源**
- [Scheduler.ets:1-8](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1-L8)
- [TaskDispatch.ets:1-10](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L1-L10)
- [HarmonyInferenceClient.ets:1-14](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L1-L14)

**章节来源**
- [Scheduler.ets:1-8](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1-L8)
- [TaskDispatch.ets:1-10](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L1-L10)
- [HarmonyInferenceClient.ets:1-14](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L1-L14)

## 性能考量
- 评分计算复杂度：O(N)，N 为在线节点数，建议控制网络规模或定期清理离线节点
- 传输选择：小文件走 MQTT，大文件走 TCP，避免 MQTT 负载过高
- 队列串行化：SerialTaskQueue 保证推理顺序与资源安全，但吞吐受限，可结合并发策略评估
- 参数动态调整：通过 /optimization/param 实时更新权重，建议在稳定场景固定权重以减少抖动
- **新增**：状态跟踪开销：任务状态映射表和历史记录会占用内存，建议定期清理过期任务
- **新增**：重试处理器：定时器检查队列，建议合理设置检查频率避免过度消耗CPU
- **新增**：指数退避算法：通过随机抖动避免雪崩效应，提高系统稳定性
- **新增**：重试队列管理：支持活跃重试任务计数，防止重试风暴
- **更新**：移除重复状态转换记录代码后，状态初始化过程更加高效，减少了不必要的历史记录操作

## 故障排查指南
- 调度无节点
  - 检查 NodeInfoSyn 是否正确订阅 /device/status 并更新 otherNodeInfo
  - 确认本机信息已通过 sendOwnInfo 发布
- 任务未分发
  - 检查 MQTTClient 是否已连接且订阅 /task/assign
  - 确认 TaskDispatch.sendTask 成功发布消息
- 任务未执行
  - 目标设备需确保 GlobalTaskManager 已初始化，且 SerialTaskQueue 正常运行
  - 检查 /task/result 是否回传成功
- 参数未生效
  - 确认 /optimization/param 消息格式正确，ParamSyn.parseParamMessage 能解析
- 网络延迟不准
  - 检查 /test/latency 流程，确保双方设备时间戳一致
- **新增**：状态查询失败
  - 检查 taskStatusMap 是否正确初始化
  - 确认状态监听器注册成功
- **新增**：重试无效
  - 检查 retryConfig 配置是否正确
  - 确认重试处理器是否正常启动
  - 验证原始任务数据是否完整保存
  - 检查指数退避算法是否按预期工作
- **新增**：重试队列异常
  - 检查重试队列长度和活跃重试任务计数
  - 确认重试处理器定时器状态
  - 验证重试回调函数是否正确设置
- **更新**：状态历史记录异常
  - 确认移除重复状态转换记录代码后，状态历史记录是否仍然完整
  - 检查状态初始化过程中的历史记录添加逻辑

**章节来源**
- [NodeInfoSyn.ets:100-170](file://entry/src/main/ets/manager/broker/monitor/NodeInfoSyn.ets#L100-L170)
- [TaskDispatch.ets:107-126](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L107-L126)
- [MQTTClient.ets:125-145](file://entry/src/main/ets/manager/broker/MQTTClient.ets#L125-L145)
- [GlobalTaskManager.ets:16-21](file://entry/src/main/ets/manager/worker/GlobalTaskManager.ets#L16-L21)
- [SerialTaskQueue.ets:51-83](file://entry/src/main/ets/manager/worker/SerialTaskQueue.ets#L51-L83)

## 结论
Scheduler 通过系统化的设备信息采集、可调权重评分与智能传输选择，实现了高效的分布式任务调度。**新增的完整任务状态管理系统**为整个调度流程提供了强大的可观测性和可靠性保障，包括8种状态跟踪、实时历史记录和智能重试机制。**新增的智能重试机制**通过指数退避算法、失败类型分类和重试队列管理，显著提升了系统的容错能力和稳定性。**更新的代码清理**移除了重复的状态转换记录代码，优化了状态管理的简洁性和可靠性。

配合串行执行队列与 MQTT 事件机制，既保证了任务的可靠分发与执行，又提供了清晰的扩展点（如动态权重、TCP 大文件传输、状态监控和重试策略）。建议在生产环境中结合网络状况与业务负载，合理设置权重与阈值，并持续监控节点状态与任务时延。

## 附录

### 使用示例与最佳实践
- 初始化与连接
  - 通过 HarmonyInferenceClient.init 配置 MQTT、加载模型、启动设备状态同步
  - 确保 MQTTClient 已连接并订阅必要主题
- 提交任务
  - 使用 HarmonyInferenceClient.submitTask(task) 提交推理任务
  - 任务将由 Scheduler 决策本地或远程执行
  - **新增**：通过 addTaskStatusListener 监听任务状态变化
- 动态权重
  - 通过 MQTT 主题 /optimization/param 下发权重数组，ParamSyn 自动更新
- 大文件传输
  - 当任务数据超过阈值时，TaskDispatch 自动切换 TCP 传输，注意目标设备需建立 TCP 连接
- 结果处理
  - 监听 /task/result，HarmonyInferenceClient.handleTaskResult 会将结果缓存并返回给调用方
- **新增**：任务状态查询
  - 使用 getTaskStatus(taskId) 获取单个任务状态
  - 使用 getAllTaskStatus() 获取所有任务状态
  - 使用 getTaskTransitionHistory(taskId) 获取任务历史记录
- **新增**：重试配置管理
  - 使用 setRetryConfig() 配置重试策略
  - 使用 getRetryConfig() 获取当前配置
  - 支持动态调整最大重试次数、基础延迟时间等参数
- **新增**：失败类型处理
  - 根据不同失败类型采取针对性处理策略
  - 使用 handleSchedulingFailure、handleLocalExecutionFailure 等方法
- **新增**：手动重试
  - 使用 canRetryTask() 检查是否可以重试
  - 使用 retryTask() 手动触发重试
- **新增**：重试队列监控
  - 使用 getRetryQueueLength() 监控重试队列长度
  - 使用 notifyRetryComplete() 通知重试完成
- **更新**：状态管理优化
  - 利用移除重复状态转换记录代码后的高效状态初始化
  - 通过优化的状态历史记录提供准确的任务生命周期追踪
- 最佳实践
  - 固定权重：在稳定场景固定权重，减少调度抖动
  - 节点治理：定期清理长时间无响应的节点，降低评分计算开销
  - 超时与重试：结合任务超时策略与 MQTT QoS，提升鲁棒性
  - **新增**：状态监控：合理设置状态清理策略，避免内存泄漏
  - **新增**：重试策略：根据业务特性调整重试次数和延迟策略
  - **新增**：指数退避：合理配置基础延迟和最大延迟，避免重试风暴
  - **新增**：失败分类：针对不同失败类型优化处理策略，提高系统稳定性
  - **更新**：状态管理：利用优化后的状态初始化流程，提高系统效率

**章节来源**
- [HarmonyInferenceClient.ets:163-196](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L163-L196)
- [HarmonyInferenceClient.ets:309-353](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L309-L353)
- [TaskDispatch.ets:107-126](file://entry/src/main/ets/manager/broker/task/TaskDispatch.ets#L107-L126)
- [ParamSyn.ets:25-37](file://entry/src/main/ets/manager/broker/paramOpt/ParamSyn.ets#L25-L37)
- [Scheduler.ets:432-467](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L432-L467)
- [Scheduler.ets:636-765](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L636-L765)
- [Scheduler.ets:753-781](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L753-L781)
- [Scheduler.ets:846-903](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L846-L903)
- [Scheduler.ets:1039-1056](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L1039-L1056)