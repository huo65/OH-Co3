...
## 更新摘要
**所做更改**
- 更新了 TaskDispatch 中的构造函数实现，现在使用空 ArrayBuffer 实例进行初始化
- 新增了 InferenceDataUtils.estimateInputDataSize 函数，专门用于统一数据结构的大小估算
- 重构了 extractFileData 方法，现在直接处理二进制缓冲区而非性能考量
- 更新了文件大小估算逻辑，使用新的 InferenceDataUtils.estimateInputDataSize 函数
- 增强了数据提取逻辑，现在完全重写以直接处理二进制缓冲区
- 优化了任务传输数据类的初始化过程，确保正确的空缓冲区处理
...
**更新** 本次更新反映了任务分发系统的最新优化：重构了数据提取逻辑以直接处理二进制缓冲区，引入了专门的 InferenceDataUtils.estimateInputDataSize 函数进行统一数据大小估算，优化了 TaskTransmitData 的初始化过程，使用空 ArrayBuffer 实例确保正确的二进制数据处理。
...
**更新** 本次更新优化了大文件传输逻辑，现在在调用 `FileTransferManager.transferFile` 时显式指定协议名称为 `'TCP'`，确保大文件传输使用 TCP 协议而非自动选择。同时使用新的 TransferNotifyCallback 类型进行统一的通知回调处理，并引入了精确的数据大小估算函数。
...
**更新** 本次更新反映了通知回调系统的重构依赖关系：TaskDispatch 和 FileTransferManager 现在都依赖于统一的 TransferNotifyCallback 类型。同时新增了 InferenceDataUtils 作为数据估算工具的依赖。
