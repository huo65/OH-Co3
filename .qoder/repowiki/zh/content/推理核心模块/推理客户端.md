...
## 更新摘要
**所做更改**
- 新增完整的任务重试机制实现，包括重试回调、资源清理、配置管理和失败场景处理
- 更新定时器泄漏防护机制，增强设备状态同步管理
- 新增重试配置管理接口，支持动态调整重试策略
- 更新 destroy() 方法的资源清理流程，包含重试资源清理
- 增强故障排查指南中关于重试机制的内容
...
**更新** 本次更新重点关注任务重试机制的完整实现，包括重试回调、资源清理、配置管理和失败场景处理。新增了重试配置管理接口，支持动态调整重试策略，包括最大重试次数、基础延迟时间、最大延迟时间和指数退避策略。同时增强了定时器泄漏防护机制，确保长时间运行的客户端不会因定时器累积而出现内存泄漏问题。
...
## 核心组件
- HarmonyInferenceClient：推理客户端主入口，负责初始化、连接管理、任务提交、系统状态查询、任务状态跟踪与**完整的任务重试机制**。
- MQTTClient：MQTT 异步客户端，负责连接、订阅、发布与事件转发。
- Scheduler：任务调度器，基于系统状态与参数权重选择最优节点执行任务，**实现完整的任务重试机制**。
...
- 初始化流程：设置 MQTT 客户端、事件监听器、模型初始化、全局任务队列初始化、**设置重试回调**、设备状态同步启动。
...
- **新增** 重试机制：实现完整的任务重试流程，包括重试回调、资源清理、配置管理和失败场景处理。
...
-pendingTaskMap : Map~string, InferenceTask~
-syncIntervalId : number | null
-retryListenerId : number | null
...
- **新增** 任务重试机制：实现完整的重试流程，包括重试配置管理、延迟计算、队列调度和回调处理。
...
- 初始化配置
  ...
  - 初始化：调用 init(config, modelName, customWorker?)，内部完成 MQTT 初始化、事件监听、模型初始化、全局队列初始化、**设置重试回调**与设备状态同步。
...
- **新增** 资源清理
  - destroy()：关闭 MQTT、释放 Worker、移除事件监听、停止设备状态同步、**清理重试资源**。
  - cleanupRetryResources()：清理 Scheduler 中的重试队列和待处理任务映射。
...
- **新增** 任务重试机制
  - setupRetryCallback()：设置重试回调，当 Scheduler 触发重试时重新提交任务。
  - resubmitTask(task)：重新提交任务，支持本地和远程执行的重试。
  - setRetryConfig(config)：设置重试配置，包括最大重试次数、基础延迟、最大延迟和指数退避。
  - getRetryConfig()：获取当前重试配置。
  - getTaskRetryCount(taskId)：获取任务当前重试次数。
  - canRetryTask(taskId)：判断任务是否可以重试。
  - retryTask(taskId)：手动触发任务重试。
  - cleanupRetryResources()：清理重试相关资源，包括重试队列和待处理任务映射。
...
- **新增** 重试机制性能：Scheduler 使用轮询机制处理重试队列，每200ms检查一次，避免频繁轮询造成CPU开销。指数退避策略结合随机抖动，避免重试风暴。
- **新增** 资源管理：destroy() 方法确保所有资源得到正确清理，包括重试队列、定时器和事件监听器。

**更新** 新增定时器泄漏防护机制，确保长时间运行的客户端不会因定时器累积而出现内存泄漏问题。日志标准化改进提升了系统的可观测性和故障排查效率。重试机制采用指数退避策略和随机抖动，有效避免重试风暴，提高系统的稳定性。
...
- 连接断开
  ...
  - 查看 `[HarmonyInference] [MQTTClient]` 前缀的日志获取连接状态详情。
...
- **新增** 重试机制问题
  - 检查重试配置是否合理，包括最大重试次数、基础延迟和指数退避设置。
  - 确认重试回调函数是否正确设置，查看 `[HarmonyInference] [Client]` 前缀的日志获取重试状态。
  - 验证重试队列是否正常工作，检查 Scheduler 的重试处理器状态。
  - 查看 `[HarmonyInference] [Scheduler]` 前缀的日志获取重试处理详情。
  - 参考路径：[Scheduler 重试处理:910-979](file://entry/src/main/ets/manager/scheduler/Scheduler.ets#L910-L979)

- **新增** 重试资源清理问题
  - 如果发现重试队列堆积或内存泄漏，检查 destroy() 方法是否正确调用。
  - 确认 cleanupRetryResources() 是否被调用，清理重试队列和待处理任务映射。
  - 验证重试处理器是否正确停止，检查 activeRetryCount 是否为0。
  - 查看 `[HarmonyInference] [Client]` 前缀的日志获取资源清理状态。
  - 参考路径：[HarmonyInferenceClient 重试资源清理:589-597](file://entry/src/main/ets/manager/HarmonyInferenceClient.ets#L589-L597)
...
**更新** 本次更新重点加强了资源管理和定时器控制机制，通过新增的 stopDeviceStatusSync() 方法和 syncIntervalId 成员变量，有效解决了定时器泄漏问题，提高了系统的稳定性和可靠性。日志标准化改进统一了各模块的日志格式，增强了系统的可观测性和故障排查效率。**新增的完整任务重试机制**进一步提升了系统的鲁棒性，包括重试配置管理、延迟计算、队列调度和资源清理，确保在各种失败场景下都能自动恢复并继续执行任务。建议在生产环境中关注网络稳定性、任务数据大小与调度参数的动态优化，同时确保正确调用 destroy() 方法进行资源清理。
